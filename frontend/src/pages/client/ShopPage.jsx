...
const CAT_COLORS = [
  'from-blue-500 to-indigo-600', 'from-emerald-500 to-teal-600', 'from-orange-400 to-rose-500',
  'from-purple-500 to-pink-600', 'from-amber-400 to-orange-500', 'from-cyan-500 to-blue-600',
];
const CAT_ICONS = ['⚽', '🎾', '🏊', '🚴', '🏋️', '🤸', '🎿', '🏇'];

const SLIDES = [
  { bg: 'from-[#1a1a2e] to-[#6B21A8]', tag: 'OFFRE LIMITÉE', title: 'Profitez plus,\ndépensez moins !', sub: "Jusqu'à -50% sur une sélection", cta: 'Découvrir les offres', accent: '#FACC15' },
  ...
  { bg: 'from-[#1a1a1a] to-[#3643BA]', tag: 'LIVRAISON OFFERTE', title: 'Commandez dès\n500 MAD', sub: 'Recevez chez vous en 2 jours ouvrables', cta: 'En profiter', accent: '#A3E635' },
];
...
function HeroCarousel() {
  ...
  return (
    <div className={`relative w-full bg-gradient-to-r ${s.bg} text-white overflow-hidden`} style={{ minHeight: 360 }}>
      ...
      <div className="max-w-screen-2xl mx-auto px-10 py-14 flex flex-col md:flex-row items-center gap-10">
        <div className="flex-1">
          <span className="inline-block text-[11px] font-bold tracking-widest px-3 py-1 rounded-full mb-4" style={{ background: s.accent, color: '#000' }}>{s.tag}</span>
          ...
          <button className="px-8 py-3 rounded-full font-bold text-sm hover:opacity-90 transition-opacity" style={{ background: s.accent, color: '#000' }}>{s.cta}</button>
        </div>
...
function ProductCard({ product, onAdd, addingId, successId, onDetail }) {
  ...
  return (
    <div onClick={() => onDetail(product.id)}
      ...
      <div className="relative h-48 bg-gray-50 flex items-center justify-center overflow-hidden">
        ...
        {IMG(product.image) ? (
          <img src={IMG(product.image)} alt={product.name} className="w-full h-full object-contain p-2 group-hover:scale-105 transition-transform duration-300" />
        ) : (
      ...
      <div className="p-3">
        ...
        <div className="mt-2"><span className="text-base font-bold text-gray-900">{parseFloat(product.price).toFixed(2)} MAD</span></div>
        <button
          onClick={(e) => { e.stopPropagation(); onAdd(product.id); }}
          disabled={product.stock === 0 || isAdding}
          className={`mt-3 w-full py-2 rounded text-sm font-semibold transition-colors flex items-center justify-center gap-2 ${
        ...
        >
          {isAdding ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> :
           isSuccess ? <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7"/></svg>Ajouté !</> :
      ...
      <div ref={productsRef} className="max-w-screen-2xl mx-auto px-6 py-10">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border border-dashed border-gray-200 rounded-2xl">
            ...
            <button onClick={() => { setSearch(''); setSelectedCategory(''); }} className="mt-4 text-[#3643BA] text-sm underline">Réinitialiser</button>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {filtered.map((product) => (
              <ProductCard key={product.id} product={product}
                onAdd={handleAddToCart} addingId={addingId} successId={successId}
                onDetail={(id) => navigate(`/shop/product/${id}`)} />
            ))}
          </div>
        )}
      </div>
